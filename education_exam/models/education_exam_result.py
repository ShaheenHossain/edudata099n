...
class EducationExamResultsNew(models.Model):
    ...
    def calculate_result(self,exam_id):
        old_results = self.env['education.exam.results.new'].search([('exam_id','=',exam_id.id)])
        if len(old_results)>0:
            for old_result in old_results:
                old_subject_line=self.env['result.subject.line.new'].search([('result_id','=',old_result.id)])
                for old_subject in old_subject_line:
                    old_paper_line=self.env['result.paper.line'].search([('subject_line','=',old_subject.id)])
                        for old_paper in old_paper_line:
                            old_paper.unlink
                    old_subject.unlink
                old_result.unlink
        student_list={}
        results=self.env['education.exam.valuation'].search([('exam_id',=exam_id)])
        for result in results:
            for student in result.valuation_line:
                if student.student_id not in student_list:
                    ...
                    new_result=self.create(result_data)
                if result.subject_id.subject_id not in student_list[student.student_id]:
                    student_list[student.student_id][result.subject_id.subject_id]={}
                    result_line=self.env['education.exam.result.new'].search([('exam_id','=',exam_id),('student_id','=',student.student_id)])
                    subject_line_data={
                        "subject_id"    : result.subject_id.subject_id,
                        "result_id"     :result_line.subject_id,
                        }
                subject_line=self.env['result.subject.line.new'].search([('')])



            subject_line_data={}
            self.env['result_subject_line_new'].create(subject_line_data)

            paper_line_data={}
            self.env['result.paper.line'].create(paper_line_data)
                ...
                if result.state!='done':
                    ...
                    for subject in result.subject_line:
                        if subject.subject_id in result.student_history.optional_subjects:
                        ...
                        elif subject.subject_id not in result.student_history.optional_subjects:
